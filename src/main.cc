...
int generate_command(int argc, char* argv[])
{
    ...
    while ((opt = getopt_long(argc, argv, "g:t:p:o:h",
                options, &long_index))
        != -1) {
        switch (opt) {
        case 'g':
    ...
    std::unique_ptr<DirectDependenciesOracle> oracle { new DirectDependenciesOracle { tests, generator_params } };
...
int deps_command(int argc, char* argv[])
{
    ...
    while ((opt = getopt_long(argc, argv, "i:a:o:m:h",
                options, &long_index))
        != -1) {
        switch (opt) {
        case 'i':
...
{
    std::cout
        << "A tool for simulating test suite dependency detection" << std::endl
...
void print_deps_help(const char* prog_name)
{
    std::cout
        ...
        << std::endl
        << "Flags:" << std::endl
        << "  -i, --input file      The file containing the synthetic test suite (Required)" << std::endl
        << "  -a, --algorithm algo  The algorithm to use to find dependencies. (Required)" << std::endl
        << "                        The possible values are: pradet, pfast, mem-fast." << std::endl
        << "  -o, --output file     The file to store dependency detection results. (Default: stdout)" << std::endl
        << "  -m, --metrics file    The file to store metrics about the run." << std::endl
...
{
    std::cout
        << "Generate a synthetic test suite using some heuristic."
        << std::endl
        << std::endl
        << "Usage: " << std::endl
        ...
        << std::endl
        << "Flags:" << std::endl
        << "  -g, --generator gen  The strategy to generate edges in the dependency graph. (Required)" << std::endl
        << "                       The possible values are: erdos-renyi, barabasi-albert, out-degree." << std::endl
        << "  --max-out n          The maximum outgoing degree in the out-degree generator. (Default: 3)" << std::endl
        << "  --min-out n          The minimum outgoing degree in the out-degree generator. (Default: 3)" << std::endl
        << "  -p, --probability n  The probability to use in Erdos-Renyi edge generation. (Default: log(nodes)/nodes)" << std::endl
        << "  -o, --output file    The file to store the synthetic test suite. (Default: stdout)" << std::endl
        << "  -t, --tests n        The number of tests to generate into the test suite. (Required)" << std::endl
        << "  -h, --help           Display this help page." << std::endl
        << std::endl;
}
