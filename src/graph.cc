...
ErdosRenyiGenerator::ErdosRenyiGenerator(const std::vector<uint32_t>& nodes,
    ...
    : GraphGenerator{nodes},
      prob{p == std::numeric_limits<double>::infinity() ? log(nodes.size())/nodes.size() : p}
{ }
...
BarabasiAlbertGenerator::BarabasiAlbertGenerator(const std::vector<uint32_t>& nodes)
    : GraphGenerator{nodes}
{ }
...
void OutDegreeGenerator::generate_edges()
{
    ...
    for (const auto& it : graph) keys.push_back(it.first);
...
std::ostream& operator<<(std::ostream& os, const Graph& g)
{
    ...
    for (const auto& it : g) os << "    " << it.first << ";" << std::endl;
