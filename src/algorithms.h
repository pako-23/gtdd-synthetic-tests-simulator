...
public:
  Metrics(uint64_t n)
      : n{n}, test_suite_runs{0}, test_runs{0}, optimal_longest_schedule{0},
        longest_schedule{0}, optimal_total_cost{0}, total_cost{0} {}
...
public:
  GraphResult(const std::vector<uint32_t> &tests) : g{tests} {};
  virtual ~GraphResult() {}
...
public:
  virtual ~Algorithm(){};
  virtual std::unique_ptr<Result> run(const std::vector<uint32_t> &,
                                      TestSuiteOracle *, Metrics &) = 0;
};

class PFAST : public Algorithm {
public:
  std::unique_ptr<Result> run(const std::vector<uint32_t> &, TestSuiteOracle *,
                              Metrics &);
};
...
private:
  void exhaustive_search(std::set<uint32_t> &,
                         std::map<uint32_t, std::set<schedule>> &,
                         TestSuiteOracle *, Metrics &);
};
