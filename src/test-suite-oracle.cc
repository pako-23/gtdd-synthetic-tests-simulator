    ...
    : graph_generator {}
{
    if (params.generator_type == "erdos-renyi")
        graph_generator.reset(new ErdosRenyiGenerator { nodes, params.probability });
    else if (params.generator_type == "barabasi-albert")
        graph_generator.reset(new BarabasiAlbertGenerator { nodes });
    else if (params.generator_type == "out-degree")
        graph_generator.reset(new OutDegreeGenerator { nodes, params.min_out, params.max_out });
    else {
        std::cerr << params.generator_type
                  << " is not a valid method to generate graphs.";
        exit(EXIT_FAILURE);
    }
...
DirectDependenciesOracle::run_tests(const std::vector<uint32_t>& tests)
{
    ...
    for (auto it1 = tests.begin(); it1 != tests.end(); ++it1) {
        std::unordered_set<uint32_t> deps = graph_generator->get_graph().get_dependencies(*it1);
